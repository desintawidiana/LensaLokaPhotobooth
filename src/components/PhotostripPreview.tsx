...
export const PhotostripPreview = forwardRef<
  HTMLDivElement,
  ...
  ref) =>
  {
    ...
    const handlePointerMove = (e: React.PointerEvent) => {
      ...
      // Calculate percentage based on pointer position relative to the container
    ...
    return (
      <div
        ...
        style={{
          ...
          touchAction: interactive ? 'none' : 'auto' // Prevent scrolling while dragging
        }}
        ...
        {/* Photos Layer */}
        <div
          className={`absolute inset-0 flex ${template.frameCount > 4 && isIG ? 'flex-wrap justify-center content-start pt-16 gap-2 px-4' : 'flex-col items-center justify-start pt-12 gap-4'} pointer-events-none`}>
          ...
          {Array.from({
            ...
            return (
              <div
                key={i}
                className={`relative bg-gray-200 overflow-hidden shadow-inner flex-shrink-0 ${isOval ? 'rounded-[50%]' : 'rounded-sm'}`}
                style={{
        ...
        <div
          key={sticker.id}
          className={`absolute select-none ${interactive ? 'cursor-grab active:cursor-grabbing' : 'pointer-events-none'}`}
          style={{
            left: `${sticker.x}%`,
            top: `${sticker.y}%`,
        ...
        <div
          key={text.id}
          className={`absolute select-none whitespace-nowrap ${interactive ? 'cursor-grab active:cursor-grabbing' : 'pointer-events-none'}`}
          style={{
            left: `${text.x}%`,
            top: `${text.y}%`,
